class Site::Profile::MyDataController < Site::ProfileController
    ...
    def update
        if @profile_member.update(params_member_profile)
            redirect_to edit_site_profile_my_datum_path(current_member.id), notice: "Update Successfull"
        else
            redirect_to :edit
        end
    end
